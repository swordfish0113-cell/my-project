...
默认生成传统叙述式法律文书。2025 年 67 类起诉状、答辩状示范文本属于要素式/表格式官方文本，默认仅用于核对诉讼要素、事实要素、证据要素和特别提示，不得直接把要素式表格作为默认输出格式。只有用户明确要求“要素式诉状”“表格式诉状”“按 2025 示范文本填写”“按法院要素式模板生成”时，才按 2025 要素式结构输出。
...
明确当事人、管辖、案由、请求事项、事实理由、证据和诉讼费用承担。请求事项应具体、明确、可执行，事实与证据逐项对应。起诉状、答辩状默认写成传统诉状：分段叙述事实与理由，避免直接输出勾选框、表格栏位或“要素式填表”格式；可使用 2025 要素式示范文本反向检查是否遗漏必要事实和证据。
...
检查标题、提交对象、当事人身份、请求事项、事实理由、证据支撑、法律依据、未核实事实、结果承诺、落款日期、刑事合规、模板属性、占位符和律师复核提示。额外检查输出形态：用户未明确要求要素式时，不得输出 2025 要素式表格、勾选框或填表式正文。
...
默认使用传统文书结构：标题、当事人信息、接收对象、请求或核心意见、事实与理由、证据清单、法律依据、风险提示、待补充信息、落款、日期。对于起诉状、答辩状，默认采用传统诉状写法，事实与理由应以审慎、连贯的自然段表达。官方模板另有结构时，应先判断该结构是否为用户要求的输出形态；2025 年 67 类要素式示范文本默认只作要素核对和结构参考，除非用户明确要求要素式，否则不得机械套用。
